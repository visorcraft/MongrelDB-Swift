# Contributing to MongrelDB Swift

Thanks for taking the time to help the MongrelDB Swift client. This document
...
The MongrelDB Swift client uses a standard **fork → branch → pull request**
workflow on GitHub.

1. **Fork** [`visorcraft/MongrelDB-Swift`](https://github.com/visorcraft/MongrelDB-Swift)
   to your GitHub account.
2. **Clone** your fork and add the upstream remote:

   ```sh
   git clone git@github.com:<you>/MongrelDB-Swift.git
   cd MongrelDB-Swift
   git remote add upstream https://github.com/visorcraft/MongrelDB-Swift.git
   ```
...
5. **Open a pull request** against `master` on `visorcraft/MongrelDB-Swift`.
...
```sh
MONGRELDB_URL=http://127.0.0.1:8453 swift test
```
...
- **Version.** Swift 5.9 or newer. Don't drop the minimum casually.
- **Dependencies.** No runtime dependencies — only the Swift standard
  library and Foundation (`URLSession`). New dependencies must be MIT or
  Apache-2.0 licensed and justified.
- **Errors.** Throw a typed error hierarchy (`MongrelDBError` base,
  `AuthError`, `NotFoundError`, `ConflictError`, `QueryError`) carrying
  the HTTP status and decoded server envelope, not generic errors.
- **Async.** Use Swift concurrency (`async`/`await`) for I/O.
- **Naming.** Idiomatic Swift: `camelCase` methods and properties,
  `PascalCase` types.
...
- The MongrelDB Swift client version (from `Package.swift` / git tag).
- Your Swift version (`swift --version`), platform, and OS.
...
The MongrelDB Swift client is dual-licensed under MIT OR Apache-2.0. By
